<template>
  ...
  <div class="box" :style="{ width: boxWidth + 'px', height: boxHeight + 'px' }">
    ...
    <div class="bottomMove" :style="{ left: mx + 'px', top: my + 'px', width: moveBottomW + 'px', height: moveBottomH + 'px' }"></div>
  </div>
</template>
...
const moveBall = () => {
  ...
  if (state.y >= offsetTop - offsetHeight && state.y <= offsetTop + offsetHeight && state.x >= offsetLeft && state.x < offsetLeft + offsetWidth) {
    map.y = -speed;
  }
  ...
  Array.from(state.arr).forEach((item, index) => {
    ...
    if (state.x > offsetLeft && state.x < offsetLeft + offsetWidth && state.y > offsetTop && state.y < offsetTop + offsetHeight) {
